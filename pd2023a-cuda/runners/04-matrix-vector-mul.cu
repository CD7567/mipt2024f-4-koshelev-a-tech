...
int main(int argc, char** argv) {
    ...
    HANDLE_ERROR(cudaMalloc((void**) &dev_B, WIDTH * sizeof(float)));
    HANDLE_ERROR(cudaMalloc((void**) &dev_result, HEIGHT * sizeof(float)));
    ...
    HANDLE_ERROR(cudaMemcpy(dev_B, B, WIDTH * sizeof(float), cudaMemcpyHostToDevice));
    ...
    MatrixVectorMul<<<grid_size, block_size>>>(WIDTH, HEIGHT, dev_A, dev_B, dev_result);

    HANDLE_ERROR(cudaMemcpy(result, dev_result, HEIGHT * sizeof(float), cudaMemcpyDeviceToHost));
    ...
    fprintf(csv_output, "MatrixVectorMul, %d, %d, %f", LENGTH, BLOCK_SIZE * BLOCK_SIZE, elapsed_time);
    fclose(csv_output);
